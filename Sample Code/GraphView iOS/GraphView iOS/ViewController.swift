...
//        graph2.subtitle        = "λ = 530nm"
        ...
        graph3.graphType       = .line
        graph3.isAutoscaling   = false
        graph3.visibleRange    = (0)...110
        graph3.sampleSize = .small
...
//    }
    ...
    var buff: [Float] = [Float](repeating: 50.0, count: 200)
    
    @objc func update() {
//        let sineValue = sin(i * delta)
        buff.append(delta)
        buff.removeFirst()
//        graph1.add(sample: sawtooth(for: i))
//        graph2.add(sample: sineValue + sin(i * 5) / 3.0)
        graph3.replace(with: buff)
